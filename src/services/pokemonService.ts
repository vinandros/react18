...
async function getPokemonsService() {
  ...
  try {
    const { data, status } = await axios.get(`${process.env.REACT_APP_API_POKE_BASE_URL}`, {
      headers: {
...
async function getPokemon(id: number | string): Promise<Pokemon> {
  ...
  try {
    const { data, status } = await axios.get(`${process.env.REACT_APP_API_POKE_BASE_URL}/${id}`, {
      headers: {
        'Access-Control-Allow-Origin': '*',
        'Content-Type': 'application/json'
      }
    ...
    if (status === 200) {
      result = new Pokemon(
        ...
        `${process.env.REACT_APP_API_POKE_BASE_URL}/${data.id}`
      );
    }
  } catch (error) {
    console.log(error);
